...
test_that("Wrong parameters in quantile function", {
  ...
  expect_warning(expect_true(is.nan(qmpb(0.2, 5, 3, -1))))
})

test_that("Wrong parameters in RNG function", {
  expect_error(rmpb(-1, 5, 3, 20))
  expect_warning(expect_true(is.na(rmpb(1, -1, 3, 20))))
  expect_warning(expect_true(is.na(rmpb(1, 5, -1, 20))))
  expect_warning(expect_true(is.na(rmpb(1, 5, 3, -1))))
})
